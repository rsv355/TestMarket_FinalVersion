...
import controller.GetMyListingsIds_Controller;
import controller.GetMyListings_Controller;
...
public class MyListingsPanel implements HomePagePanelInterface {
	...
	private MyListingsListAdapter dataAdapter = null;
	...
	private GetMyListingsIds_Controller initialThread;
	private GetMyListings_Controller loadThread;
	...
	public void onCreate(Context context, Activity parentActivity) {
		...
		outerLayout = (RelativeLayout) parentActivity
				.findViewById(R.id.mylistingstab_outerlayout);
		progressBar = (ProgressBar) parentActivity
				.findViewById(R.id.mylistingstap_progressBar1);
		noSamples = (TextView) parentActivity
				.findViewById(R.id.mylistingstap_textView1);
		noSamples.setVisibility(View.INVISIBLE);
		listView = (ListView) parentActivity
				.findViewById(R.id.mylistingstap_listView1);
		listView.setVisibility(View.INVISIBLE);

		loadMoreView = ((LayoutInflater) parentActivity
				.getSystemService(Context.LAYOUT_INFLATER_SERVICE)).inflate(
				R.layout.mylistingslistfooter, null, false);
		...
		page1 = (TextView) parentActivity
				.findViewById(R.id.mylistingstap_page1);
		page2 = (TextView) parentActivity
				.findViewById(R.id.mylistingstap_page2);
		page3 = (TextView) parentActivity
				.findViewById(R.id.mylistingstap_page3);
		page4 = (TextView) parentActivity
				.findViewById(R.id.mylistingstap_page4);
		page5 = (TextView) parentActivity
				.findViewById(R.id.mylistingstap_page5);

		previousPage = (ImageButton) parentActivity
				.findViewById(R.id.mylistingstap_previouspage);
		nextPage = (ImageButton) parentActivity
				.findViewById(R.id.mylistingstap_nextpage);
		...
		try{
				IntentFilter receiverfilter = new IntentFilter(
						Static_view_info.MYLISTINGSIDS_BROADCAST);
		...
		try{
				IntentFilter receiverfilter1 = new IntentFilter(
						Static_view_info.MYLISTINGS_BROADCAST);
	...
	public void show() {
		...
		noSamples.setVisibility(View.INVISIBLE);
		listView.setVisibility(View.INVISIBLE);
	//	if (Static_view_info.sampleStateChanged_listings) {
			initialThread = new GetMyListingsIds_Controller(parentActivity,
					Static_view_info.userId);
			initialThread.execute();

	//		Static_view_info.sampleStateChanged_listings = false;
	//	} 
//		else {
//			if (!Static_view_info.myListingsIds.equals("null")) {
//				// get frist max mysamples ids
//				page1startingSampleIndex = 0;
//				if (page1startingSampleIndex + Static_view_info.MAX_MYLISTINGS > Static_view_info.idsSize_listings)
//					page1endingSampleIndex = Static_view_info.idsSize_listings;
//				else
//					page1endingSampleIndex = page1startingSampleIndex
//							+ Static_view_info.MAX_MYLISTINGS;
//				updatePagesCounters();
...
//		}
			...
			public void onClick(View arg0) {
				if (internetConnection.isConnectingToInternet()) {
					...
					page1startingSampleIndex = page1startingSampleIndex
							- (5 * Static_view_info.MAX_MYLISTINGS);
					page1endingSampleIndex = page1startingSampleIndex
							+ Static_view_info.MAX_MYLISTINGS;
					page1Text -= 5;
					page2Text -= 5;
			...
			public void onClick(View arg0) {
				if (internetConnection.isConnectingToInternet()) {
					...
					if (page1startingSampleIndex
							+ Static_view_info.MAX_MYLISTINGS < Static_view_info.idsSize_listings)
						page1endingSampleIndex = page1startingSampleIndex
								+ Static_view_info.idsSize_listings;
					else
						page1endingSampleIndex = Static_view_info.idsSize_listings;
					page1Text += 5;
					page2Text += 5;
	...
	private void updatePagesCounters() {
		...
		if (page1endingSampleIndex < Static_view_info.idsSize_listings) {
			...
			if (page2startingSampleIndex + Static_view_info.MAX_MYLISTINGS < Static_view_info.idsSize_listings) {
				...
				if (page3startingSampleIndex + Static_view_info.MAX_MYLISTINGS < Static_view_info.idsSize_listings) {
					...
					if (page4startingSampleIndex
						...
						page4endingSampleIndex = Static_view_info.idsSize_listings;
						page5.setVisibility(View.INVISIBLE);
						nextPage.setVisibility(View.INVISIBLE);
					}
				} else {
					page3endingSampleIndex = Static_view_info.idsSize_listings;
					page4.setVisibility(View.INVISIBLE);
					page5.setVisibility(View.INVISIBLE);
					nextPage.setVisibility(View.INVISIBLE);
				}
			} else {
				page2endingSampleIndex = Static_view_info.idsSize_listings;
	...
	private void loadPage(int startingProductId, int endingProductId) {
		...
		noSamples.setVisibility(View.INVISIBLE);
		clean();
		loadThread = new GetMyListings_Controller(parentActivity,
	...
	private void clean() {
		...
		MySampleList.clear();
		Static_view_info.mylistings_drawables.clear();
		Static_view_info.currentMyListings = null;
		dataAdapter = null;
	}
	...
	private void initiateListView() {
		if (Static_view_info.currentMyListings != null) {
			JSONArray samples_array;
			try {
				samples_array = Static_view_info.currentMyListings;
				String productId = "1";
				for (int i = 0; i < samples_array.length(); i++) {
					...
					Drawable drawable = Static_view_info.mylistings_drawables
							.get(i);
				...
				dataAdapter = new MyListingsListAdapter(parentActivity,
			...
			public void onItemClick(AdapterView<?> adapter, View view,
				...
				if (internetConnection.isConnectingToInternet()) {
					...
					new GetUserName_Controller(new ProgressDialog(
							parentActivity), parentActivity, uploaderUser, 3)
							.execute();
				} else {
					Toast.makeText(parentActivity,
							"Your are not connected to the Internet",
	...
	private void openProductPage(String username) {
		...
		Intent intent = new Intent(parentActivity, MyListingsItemPageActivity.class);
	...
	void reg(){
		IntentFilter receiverfilter = new IntentFilter(Static_view_info.MYLISTINGSIDS_BROADCAST);
		parentActivity.registerReceiver(mySamplesIds_receiver, receiverfilter);
		
		IntentFilter receiverfilter1 = new IntentFilter(Static_view_info.MYLISTINGS_BROADCAST);
		parentActivity.registerReceiver(mySamples_receiver, receiverfilter1);
		
		IntentFilter receiverfilter2 = new IntentFilter(Static_view_info.GETUSERNAME_BROADCAST);
		parentActivity.registerReceiver(getUserName_receiver, receiverfilter2);	
	}
